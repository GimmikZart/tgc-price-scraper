...
const illustrationTypes = [
  ...
  "red-manga",
  "gold-manga",
  "reprint-missing-pen-symbol-next-to-the-artist-name",
  "textured-foil",
  "other",
  "base",
];
...
async function loadFile(name) {
  ...
  cards.value = list;
  onlyPxCards.value = list.filter((card) => typeof card.imageId === "string");
}
...
const currentCard = computed(() => {
  ...
  return withStoredCardImage(currentRawCard.value, imageOptions.value);
});
...
function downloadJson() {
  ...
  anchor.download = selectedFile.value
    ...
    <div class="h-full overflow-auto">
      ...
      <div v-if="currentCard" class="grid grid-cols-2 gap-6 px-2 pb-10">
        ...
        <div class="grid grid-cols-2 gap-2 px-3">
          <v-btn
            v-for="type in illustrationTypes"
            :key="type"
            class="px-3 py-2 rounded border hover:opacity-80"
            @click="setIllustration(type)"
          >
            {{ type }}
          </v-btn>
        </div>
      ...
      <div v-if="selectedFile" class="flex flex-col md:flex-row items-center gap-5 pt-4 border-t">
        ...
        <v-btn class="px-3 py-2 rounded border" @click="downloadJson">Download new JSON</v-btn>
      </div>
    </div>
  </section>
</template>
