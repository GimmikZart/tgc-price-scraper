...
begin
  if not exists (
    ...
    create type public.match_result as enum ('won', 'lost');
  end if;
end
$$;
...
begin
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where t.typname = 'match_cancel_reason'
      and n.nspname = 'public'
  ) then
    create type public.match_cancel_reason as enum ('canceled', 'rejected');
  end if;
end
$$;
...
create table if not exists public.matches (
  ...
  opponent_deck jsonb,
  challenger_result public.match_result,
  opponent_result public.match_result,
