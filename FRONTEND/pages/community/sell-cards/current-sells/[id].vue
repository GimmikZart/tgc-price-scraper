...
async function loadListing() {
  ...
  try {
    const sellListings = await fetchLoggedUserSellListings();
    listing.value = sellListings.find((sellListing) => String(sellListing?.id) === listingId.value) ?? null;
  } catch (error) {
    listing.value = null;
...
<template>
  <section class="relative h-full">
    <Toolbar label="Proposte per" fixed back-button>
      <template #info>
        <p v-if="isLoading" class="sell-state-message">Caricamento dettagli vendita...</p>
        <p v-else-if="!listing || !hasListingCard" class="sell-state-message">Vendita non trovata</p>
        ...
        <div v-else class="flex w-full gap-3">
          ...
          <div class="h-full w-full flex-1 flex flex-col gap-2">
            <div>
              <p class="sell-card-name line-clamp-2">{{ listing.card.name }}</p>
              <p class="sell-card-copies mb-2">{{ copiesInSale }} copie in vendita</p>
            </div>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-3 pb-2">
      <p v-if="isLoading" class="sell-state-message">Caricamento proposte in acquisto...</p>
      <div v-else class="proposal-shell h-[600px]">
        <p class="sell-state-message">Nessuna proposta disponibile al momento</p>
      </div>
    </div>
