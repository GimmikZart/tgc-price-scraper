...
export function useScraperStream() {
  ...
  const connect = () => {
    ...
    eventSource.addEventListener("scraping_product_success", (e) => {
      ...
      snackbar.addMessage(
        `Scraping riuscito per ${set} di ${store}`,
        "success"
      );
    });
    ...
    eventSource.addEventListener("scraping_product_error", (e) => {
      ...
      snackbar.addMessage(
        `Errore scraping per ${set} di ${store}`,
        "error",
        error
      );
    });
    ...
    eventSource.addEventListener("generic_error", (message) => {
      snackbar.addMessage(
        `Errore durante lo scraping: ${message.data}`,
        "error"
      );
    });
    ...
    eventSource.addEventListener("generic_warning", (message) => {
      snackbar.addMessage(message.data, "warning");
    });
    ...
    eventSource.addEventListener("scraping_completed", (e) => {
      ...
      snackbar.addMessage(
        "Scraping completato",
        "info",
        `Durata: ${duration}s | RAM: ${ramUsed} MB`
      );
    });
