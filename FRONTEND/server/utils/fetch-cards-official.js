...
export default async function scrapeCardsOfficial({ expansionName }) {
  ...
  try {
    ...
    await page.waitForSelector(
      "#frmSearch > div.formsetDefaultArea > div.seriesCol > button",
      { visible: true }
    );
    await page.click(
      "#frmSearch > div.formsetDefaultArea > div.seriesCol > button"
    ...
    try {
      ...
      fs.writeFileSync(path.join(debugDir, "modal-not-found.html"), html);
      throw new Error(
    ...
    for (const item of modalItems) {
      const txt =
        (await page.evaluate((el) => el.textContent?.trim(), item)) || "";
    ...
    const foundSetText = await page.evaluate(
