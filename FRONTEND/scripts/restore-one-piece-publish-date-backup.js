...
main().catch((error) => {
  console.error("Errore irreversibile durante il restore del backup publish_date:", error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  const cards = buildGameCatalogFromSetFiles(workspace.setFiles);
  ...
  console.log("");
  console.log("Restore publish_date completato.");
  console.log(
    JSON.stringify(
      {
        backup: sourceDir,
        restoredCardsDir: resolveGameLocalPaths(GAME_SLUG).cardsDir,
        syncSummary,
      },
      null,
      2,
