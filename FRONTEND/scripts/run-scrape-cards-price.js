...
import { CARD_TRADER_SERVICE_NAME, ensureCardTraderSlug } from "../utilities/cardTraderSlug.js";
...
export async function main() {
  log("start", `GAME=${GAME_SLUG} MAX_AGE_HOURS=${MAX_AGE_HOURS} FORCE_ALL=${FORCE_ALL} HEADLESS=${SCRAPE_HEADLESS} MAX_FILES=${MAX_FILES || "all"}`);
    ...
    for (let fileIndex = 0; fileIndex < filesToProcess.length; fileIndex += 1) {
      ...
      for (let cardIndex = 0; cardIndex < cards.length; cardIndex += 1) {
        ...
        if (!hasCardTraderSlug) {
          const nextCard = ensureCardTraderSlug(card, { preserveVerified: true });
          if (nextCard !== card) {
        ...
        for (let slugIndex = 0; slugIndex < card.slugs.length; slugIndex += 1) {
          ...
          const isAutoGenerated = Boolean(slug?.autoGenerated);
          const url = slug?.url;
          if (!isCardTrader || !url || (!isVerified && !isAutoGenerated)) continue;
